...
function AdminViewAllInstructors() {
    const [adminAllInstructors, setAdminAllInstructors] = useState([]);
    ...
    useEffect(() => {
        const fetchAllInstructors = async () => {
            try {
                ...
                const response = await axios.get('http://127.0.0.1:8000/api/admins/users-management/instructors', {
                    headers: {
                ...
                if (Array.isArray(response.data)) {
                    ...
                    if (response.data.instructors) {
                        ...
                        instructorsData = response.data.results;
                    } else {
                        instructorsData = [];
                    }
                } else {
                ...
                setAdminAllInstructors(Array.isArray(instructorsData) ? instructorsData : []);
            } catch (error) {
    ...
    const confirmDeleteUser = async () => {
        try {
            ...
            setAdminAllInstructors(prevInstructors =>
                prevInstructors.filter(c => c.id !== deleteModal.user.id)
            );
    ...
    return (
        <>
            ...
            <div id="main-container" className="container-fluid py-4" style={{ marginLeft: '260px', maxWidth: 'calc(100% - 280px)' }}>
                            ...
                            <div className="bg-dark bg-gradient p-4">
                                <div className="d-flex justify-content-between align-items-center">
                                    <div className="d-flex align-items-center">
                                        ...
                                        <div>
                                            <h1 className="m-0 fs-3 fw-bold text-white">All Instructors</h1>
                                            <p className="m-0 text-white-50">
                                                {adminAllInstructors.length} {adminAllInstructors.length === 1 ? 'instructor' : 'instructors'} total
                                            </p>
                                        </div>
                                    </div>
                                </div>
                ...
                {adminAllInstructors.length === 0 && !error ? (
                    <div className="card border-0 shadow-sm">
                        <div className="card-body text-center py-5">
                            <i className="fas fa-book-open fs-1 text-muted mb-3"></i>
                            <h4 className="mb-3">No Instructors Found</h4>
                            <p className="text-muted mb-4">There is no instructors yet.</p>
                        </div>
                    </div>
                ) : error ? (
                    <div className="alert alert-danger" role="alert">
                        <div className="d-flex align-items-center">
                            <i className="fas fa-exclamation-circle me-2"></i>
                            <div>
                                <h5 className="fw-bold mb-1">Error Loading Instructors</h5>
                                <p className="mb-0">{error}</p>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <tr key={user.id} className="hover-shadow">
                                                ...
                                                <td className="text-end pe-4">
                                                    <div className="d-flex justify-content-end gap-2">
                                                        <a
                                                            href={`/admin/instructors/view/${user.slug || user.id}`}
                                                        ...
                                                        <a
                                                            href={`/admin/instructors/edit/${user.id}`}
...
export default AdminViewAllInstructors;